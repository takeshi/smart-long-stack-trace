...
function getFrames(error: Error): Line[] {
  return error.stack ? StackTraceParser.parse(error.stack) : [];
}

function addErrorStack(lines: string[], error: Error): void {
  let trace: Line[] = getFrames(error);
  for (let i = 0; i < trace.length; i++) {
    const frame = trace[i];
    // Filter out the Frames which are part of stack capturing.
    if (!(i < IGNORE_FRAMES.length && IGNORE_FRAMES[i].line === frame.line)) {
      if (filter(frame)) {
        lines.push('  ' + trace[i].line);
      } else {
        lines.push(trace[i].line);
      }
    }
...
function indentstack(inputStack: any) {
  let lines = StackTraceParser.parse(inputStack);
  let stack = '';
...
function renderLongStackTrace(frames: LongStackTrace[], stack: string): string {
  const longTrace: string[] = [indentstack(stack)];
...
export const LongStackTraceSpec = <ZoneSpec>{
...
function captureStackTraces(stackTraces: Line[][], count: number): void {
  if (count > 0) {
